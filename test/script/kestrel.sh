...
HEAP_OPTS="-Xmx1024m -Xms512m -XX:NewSize=256m"
...
GC_LOG="-Xloggc:$APP_HOME/data/gc.log"
DEBUG_OPTS="-XX:ErrorFile=$APP_HOME/data/java_error%p.log"


JAVA_OPTS="-server -Dstage=$STAGE $GC_OPTS $GC_TRACE $GC_LOG $HEAP_OPTS $DEBUG_OPTS"

pidfile="$APP_HOME/data/$APP_NAME.pid"
# This second pidfile exists for legacy purposes, from the days when kestrel
# was started by daemon(1)
daemon_pidfile="$APP_HOME/data/$APP_NAME-daemon.pid"
...
case "$1" in
  start)
    ...
    # command line args.
    if [ -f $APP_HOME/data/gc.log ]; then
      mv $APP_HOME/data/gc.log $APP_HOME/data/gc_$TIMESTAMP.log;
    fi

    #ulimit -n $FD_LIMIT || echo -n " (no ulimit)"
    ulimit -c unlimited || echo -n " (no coredump)"

    sh -c "echo "'$$'" > $pidfile; echo "'$$'" > $daemon_pidfile; exec ${JAVA_HOME}/bin/java ${JAVA_OPTS} -jar ${APP_HOME}/${JAR_NAME} >> $APP_HOME/data/stdout 2>> $APP_HOME/data/error" &
    disown %-
    sleep $INITIAL_SLEEP
  ...
  *)
    echo "Usage: ./${APP_NAME}.sh {start|stop|restart|status}"
    exit 1
  ;;
esac
